...
<template>
	<li :class="$style.NavItem">
		<a :href="props.navItem.url" :class="$style.NavItem_title">{{ props.navItem.title }}</a>
		<ul v-if="isOpen">
			<LayoutSideNavItem :level="level + 1" :nav-item="child" v-for="child in props.navItem.children" />
			<template v-if="level < 2">
				<LayoutSideNavItem :level="level + 1" :nav-item="{
					title: 'Some page',
					url: '#',
					children: []
				}" v-for="_ in 5" />
